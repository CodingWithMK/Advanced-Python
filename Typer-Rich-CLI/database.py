...
conn = sqlite3.connect("Typer-Rich-CLI/todos.db")
cursor = conn.cursor()
...
def update_todo(position: int, task: str, category: str):
    with conn:
        if task is not None and category is not None:
            ...
            {"position": position, "category": task})

def complete_todo(position: int):
    with conn:
        cursor.execute("UPDATE todos SET status = 2, date_completed = :date_completed WHERE positon = :position",
